...
def insideTriangle(sensors, triSens, point):
    ...
    Area = 0.5 * (-p[1][1]*p[2][0] + p[0][1]*(-p[1][0] + p[2]
                                              [0]) + p[0][0]*(p[1][1] - p[2][1]) + p[1][0]*p[2][1])
    if Area == 0:
        return False
    t = 1/(2*Area)*(p[0][0]*p[1][1] - p[0][1]*p[1][0] + (p[0]
                                                         [1] - p[1][1])*point[1] + (p[1][0] - p[0][0])*point[2])
    s = 1/(2*Area)*(p[0][1]*p[2][0] - p[0][0]*p[2][1] + (p[2]
                                                         [1] - p[0][1])*point[1] + (p[0][0] - p[2][0])*point[2])
            ...
            for a in range(0, len(sensor.neighbours[i])):
                ...
                for j in range(0, len(sensor.neighbours)):
                    if i != j:
                        ...
                        dx = abs(
                            intrsxn[1]-sensors[sensor.neighbours[j][0][0]].x)
                        dy = abs(
                            intrsxn[2]-sensors[sensor.neighbours[j][0][0]].y)
                        distsq = dx**2+dy**2
                        ...
                        elif distsq < (sensors[sensor.neighbours[j][0][0]].r**2) + 0.0000000001:
                            isInside = True
                            sensor.neighbours[i][a] = sensor.neighbours[i][a] + \
                                (sensor.neighbours[j][0][0],)
                            break
            ...
            for k in range(0, num):
                ...
                if isBoundaryArc and len(circ) >= 2:
                    isInside = insideTriangle(
                        sensors, [i, circ[0], circ[1]], (0, xm, ym))
                    if isInside == True:
                        isBoundaryArc = False
                if isBoundaryArc:
                ...
                for k in range(0, len(ns.neighbours)):
                    for intrsxn in ns.neighbours[k]:
                        ...
                        for j in range(0, len(ns.neighbours)):
                            snj = sensors[ns.neighbours[j][0][0]]
                            sj = sensors[j]
                            sk = sensors[k]
                            if (snj.x != sensor.x or snj.y != sensor.y) and (sj.x != sk.x or sj.y != sk.y):
                                dx = abs(
                                    intrsxn[1]-sensors[ns.neighbours[j][0][0]].x)
                                dy = abs(
                                    intrsxn[2]-sensors[ns.neighbours[j][0][0]].y)
                                dist = sqrt(dx**2+dy**2)
                        ...
                        if isInside == False:
                            if (intrsxn[1], intrsxn[2]) not in sensor.maskedIntersections:
                                sensor.maskedIntersections.append(
                                    (intrsxn[1], intrsxn[2]))
    ...
    for p in scurr.maskedIntersections:
        ...
        if stratType == 0:
            try:
                ...
                pts = getLineCircleIntersection(
                    0, scurr.y, scurr.x, p[1], p[0], scurr.r, True)
            else:
                pts = getLineCircleIntersection(
                    m, scurr.y, scurr.x, p[1], p[0], scurr.r, False)
            ...
            pts = getLineCircleIntersection(
                m, scurr.y, scurr.x, p[1], p[0], scurr.r, m == 90 or m == 270)
        else:
            pts = None
        if pts == None:
...
class RegularHexagon:
    def __init__(self,centre, radius):
        ...
        x=self.centre.x
        y=self.centre.y
        self.points=[Point(x, y+r), Point(x+d, y+r/2), Point(x+d, y-r/2), Point(x, y-r), Point(x-d, y-r/2), Point(x-d, y+r/2)]
    def __getitem__(self,key):
...
def getCombinedPolygons(Hexagons):
    poly_lines=[]
    Hexagons_l=[x.getLines() for x in Hexagons]
...
def plotHexagon(hexagon):
    area.create_polygon(*[(SF*x.x, SF*x.y)
                      for x in hexagon], outline="blue",fill="black")
...
# area.after(2000,lambda : ShowSteps(msensors))
window.mainloop()
