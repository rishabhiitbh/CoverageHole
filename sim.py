...
def drawPlot(sensors):
    for sensor in sensors:
        if sensor.isBoundry == False:
            ...
            area.create_circle(sensor.x*SF,sensor.y*SF,sensor.r*SF,outline="#000",width=1)
            arcl=sensor.boundryArc[1]-sensor.boundryArc[0]
            if (arcl<0):
                arcl=arcl+360
...
def getHoleArea(sensors):
    ...
    print("area of hole is "+hArea.__str__())
...
getHoleArea(msensors)
getMaskedIntersections(msensors)
drawPlot(msensors)
#getApproxArea(msensors)
area.after(2000,lambda : repat(msensors))  
print(boundrySensors)
print("number of sensors = " + len(msensors).__str__())
print("number of intersections = " + len(intersections).__str__())
print(msensors[boundrySensors[0]].x,msensors[boundrySensors[0]].y)
window.mainloop()